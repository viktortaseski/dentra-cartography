...
function formatSelectedDate(iso: string): string {
  ...
  return `${DAY_NAMES[date.getDay()]}, ${MONTH_NAMES[month - 1]} ${day}`
}
...
export function DaySidebar({
  ...
  return (
    <div className="flex flex-col border-t border-gray-100 mt-2 pt-2 flex-1 min-h-0">
      {/* Date heading */}
      <div className="px-3 mb-2">
        <p className="text-xs font-semibold text-gray-700">{formatSelectedDate(selectedDate)}</p>
      </div>
        ...
        {dayAppointments.length === 0 ? (
          ...
          dayAppointments.map((appt) => (
            <button
              ...
              className="w-full text-left px-2 py-1.5 rounded-lg hover:bg-gray-50 border border-gray-100 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            >
              <p className="text-xs font-medium text-gray-800">
                {appt.startTime}–{appt.endTime}
              </p>
              <p className="text-xs text-gray-600 truncate">{appt.title}</p>
